...
if len(sys.argv) == 1: # Running normally
    ...
    os.system(f"cd {BASE_DIRECTORY}/{app_name} && git init && git add . && git commit -am '{MASTER_PROMPT}'")
...
while True:
    ...
    original_entered_message = EDIT_MESSAGE
    ...
    thinking_stop()
    os.system(f"cd {BASE_DIRECTORY}/{app_name} && git init && git add . && git commit -am '{original_entered_message}'")
